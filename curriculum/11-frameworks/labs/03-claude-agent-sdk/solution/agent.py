"""Lab 11.03: reference agent in the Claude Agent SDK -- reference solution.
See ../README.md.
"""

from __future__ import annotations

import ast
import operator
from collections.abc import Callable

from claude_agent_sdk import (
    AssistantMessage,
    ClaudeAgentOptions,
    TextBlock,
    create_sdk_mcp_server,
    query,
    tool,
)

_BINOPS: dict[type, Callable[[float, float], float]] = {
    ast.Add: operator.add,
    ast.Sub: operator.sub,
    ast.Mult: operator.mul,
    ast.Div: operator.truediv,
}


def _eval_node(node: ast.AST) -> float:
    if isinstance(node, ast.Constant) and isinstance(node.value, int | float):
        return node.value
    if isinstance(node, ast.BinOp) and type(node.op) in _BINOPS:
        return _BINOPS[type(node.op)](_eval_node(node.left), _eval_node(node.right))
    raise ValueError(f"Unsupported expression element: {ast.dump(node)}")


def _evaluate(expression: str) -> float:
    try:
        tree = ast.parse(expression, mode="eval")
    except SyntaxError as exc:
        raise ValueError(f"Invalid expression: {exc}") from exc
    return _eval_node(tree.body)


@tool("calculate", "Evaluate a basic arithmetic expression.", {"expression": str})
async def calculate(args: dict) -> dict:
    result = _evaluate(args["expression"])
    return {"content": [{"type": "text", "text": str(result)}]}


def build_options() -> ClaudeAgentOptions:
    server = create_sdk_mcp_server("calculator", tools=[calculate])
    return ClaudeAgentOptions(
        mcp_servers={"calculator": server},
        allowed_tools=["mcp__calculator__calculate"],
        system_prompt="Use the calculate tool to answer math questions.",
        model="claude-haiku-4-5",
    )


async def run_agent(question: str) -> str:
    final_text = ""
    async for message in query(prompt=question, options=build_options()):
        if isinstance(message, AssistantMessage):
            text_parts = [block.text for block in message.content if isinstance(block, TextBlock)]
            if text_parts:
                final_text = "".join(text_parts)
    return final_text
