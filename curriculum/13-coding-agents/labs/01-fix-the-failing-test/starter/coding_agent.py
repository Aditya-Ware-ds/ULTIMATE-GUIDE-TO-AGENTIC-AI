"""Lab 13.01: an agent that fixes a failing test in a small repo, entirely
inside shared/sandbox/. See ../README.md for the full spec.

Fill in the pieces below. Run the tests against your work with:
    LAB_TARGET=starter uv run pytest \
        curriculum/13-coding-agents/labs/01-fix-the-failing-test/tests
"""
...
def resolve_within_repo(repo_root: Path, relative_path: str) -> Path:
    ...
    resolved path is not inside `repo_root` (see lessons/02-repo-navigation-and-editing.md).
...
def build_tool_registry(repo_root: Path) -> dict[str, Callable]:
    ...
    callable that takes only the model-supplied arguments (repo_root should
    be bound via closure, not exposed as a tool argument).

    run_tests' callable should return `result.stdout + result.stderr` as a
    single string, not the raw ShellResult (that's for the final check in
    run_coding_agent below).
...
def dispatch(tool_call: ToolCall, registry: dict[str, Callable]) -> ToolResult:
    """Same contract as prior modules' dispatch: look up the tool, call it
    with tool_call.arguments, wrap the result as a ToolResult, and catch any
    exception as an is_error=True ToolResult rather than raising.
...
async def run_coding_agent(client, repo_root: Path, task: str, max_steps: int = 6) -> dict:
    ...
    re-run the tests (do not trust the model's own claim of success --
    see lessons/03-test-driven-agent-loop.md) and return:
